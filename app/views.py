...
ARQUIVO = os.path.join(BASE_DIR, "data", "livros.json")
...
@views.route("/api/livros", methods=["GET"])
def listar_livros():
    return jsonify(carregar_livros())

@views.route("/api/exportar-pdf", methods=["GET"])
def exportar_pdf():
    return exportar_livros_pdf()

@views.route("/api/livros", methods=["POST"])
def cadastrar_livro():
    dados = request.json
    # Validações
...
@views.route("/api/livros/<int:id>", methods=["GET"])
def buscar_livro(id):
    livros = carregar_livros()
    for livro in livros:
        if livro.get("id") == id:
...
@views.route("/api/livros/<int:id>", methods=["PUT"])
...
@views.route("/api/livros/<int:id>", methods=["DELETE"])
